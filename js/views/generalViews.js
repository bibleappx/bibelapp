...
export function renderSearchPage(initialQuery = '') {
    ...
    const performSearchAndDisplay = async () => {
        ...
        if (strongsRegex.test(term)) {
            if (activeFilter !== 'bible' && activeFilter !== 'all') {
                resultsInfoContainer.innerHTML = `<p class="text-tertiary">Die Suche nach Strongs-Nummern ist nur in den Filtern "Alles" oder "Bibel" möglich.</p>`;
                resultsContainer.innerHTML = '';
                paginationContainer.innerHTML = '';
                return;
            }
            
            const strongsId = term.charAt(0).toUpperCase() + term.slice(1);
            const strongsTranslationId = 'elb-csv';
            const occurrences = state.strongsVerseIndex.get(strongsTranslationId)?.get(strongsId) || [];
            
            // NEU: Strongs-Nummer im Infotext wird klickbar gemacht.
            const strongsInfoText = `<p class="text-tertiary">${occurrences.length} Vorkommen für Strongs-Nummer <strong class="strongs-mention-hover" data-strong-id="${strongsId}" style="cursor: pointer; text-decoration: underline;">${strongsId}</strong> in ELB-CSV gefunden.</p>`;
            resultsInfoContainer.innerHTML = strongsInfoText;
            
            const verseObjects = occurrences.map(ref => {
                return state.verses.find(v => v.book_number === ref.b && v.chapter === ref.c && v.verse === ref.v);
            }).filter(Boolean);

            const parsedQueryForHighlight = { highlightRegexes: [{ source: `(${utils.escapeRegExp(strongsId)})`, flags: 'gi' }] };
            resultsContainer.innerHTML = verseObjects.map(res => renderSearchResultItemHTML(res, parsedQueryForHighlight)).join('') || '<p class="text-tertiary p-3">Keine Ergebnisse gefunden.</p>';
            paginationContainer.innerHTML = '';
            return;
        }
