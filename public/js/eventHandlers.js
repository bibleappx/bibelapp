...
async function handleAppClicks(e) {
    ...
    const toggler = e.target.closest('.internal-verse-toggler');
    if (toggler) {
        e.preventDefault();
        ...
        if (contentDiv) {
            ...
            if (isHidden && contentDiv.classList.contains('unloaded')) {
                const bookNum = parseInt(toggler.dataset.book);
                const chapter = parseInt(toggler.dataset.chapter);
                const verse = parseInt(toggler.dataset.verse);
                setTimeout(() => {
                    const targetVerse = state.verses.find(v => v.book_number === bookNum && v.chapter === chapter && v.verse === v);
                    contentDiv.innerHTML = targetVerse ? `<p class="mb-0">${targetVerse.text.replace(/<pb\/>/g, '')}</p>` : '<p class="text-danger mb-0">Vers nicht gefunden.</p>';
                    contentDiv.classList.remove('unloaded');
                }, 50);
            }
        }
    }
    ...
    if (commentaryHoverTrigger) {
        ...
        hideChapterPopup();
        hideDictionaryPopup();
        if (document.querySelector('#commentary-popup.visible')) {
            hideCommentaryPopup();
        } else {
            showCommentaryPopup(commentaryHoverTrigger);
        }
        return;
    }
    const dictionaryHoverTrigger = e.target.closest('.dictionary-mention-hover');
